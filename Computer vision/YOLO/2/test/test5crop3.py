...
def main():
    ...
    while True:
        if frames_buffer:
            ...
            overlay_mask = np.ones_like(img_resized) * 255
            cv2.fillPoly(overlay_mask, [scaled_roi_polygon], (0, 0, 0))
            overlay_color = (128, 128, 128)  # Gray
            outside_roi = cv2.bitwise_and(img_resized, mask_inv)
            outside_roi = cv2.addWeighted(outside_roi, 0.7, overlay_mask, 0.3, 0)
