...
while cap.isOpened():
    ...
    for idx, frame in enumerate(frame_buffer):
        if idx % frame_skip == 0:
            ...
            person_detections = [detection for detection in detections if detection['class'] == '0']

            # Count persons within ROI and draw on frame
            frame = counter.start_counting(frame, person_detections)
