...
def main():
    ...
    while True:
        if frames_buffer:
            ...
            # Create a mask to overlay the area outside the ROI with the desired color
            overlay_mask = np.ones_like(img_resized, dtype=np.uint8) * (220, 220, 220)  # Light gray color
            cv2.fillPoly(overlay_mask, [scaled_roi_polygon], (0, 0, 0))  # Set the ROI region to black
            overlay_mask_inv = cv2.bitwise_not(overlay_mask)

            overlay_mask_inv_resized = cv2.resize(overlay_mask_inv, (resized_width, resized_height))
            outside_roi = cv2.bitwise_and(img_resized, overlay_mask_inv_resized)
            outside_roi = cv2.addWeighted(outside_roi, 1.0, overlay_mask_inv_resized, 0.5, 0)  # Adjust alpha for transparency
            ...
            inside_roi = cv2.bitwise_and(img_resized, overlay_mask)
